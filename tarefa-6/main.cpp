...
glm::mat4 getModel();
vector<glm::vec3> generateControlPointsSet(string filepath);
...
const std::string MODEL_PATH = "../models/Suzanne/SuzanneTriTextured";
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "CG Tarefa 6 - Nicole", nullptr, nullptr);
    ...
    // Trajetório
    int i = 0;
    vector<glm::vec3> controlPoints = generateControlPointsSet(CONTROL_POINTS_PATH);
    Bezier bezier;
    bezier.setControlPoints(controlPoints);
    bezier.setShader(&shader);
    bezier.generateCurve(POINTS_PER_SEGMENT);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Atualiza view e projection com câmera
        camera.updateShader(width, height, &shader);
